          ...
          this.state.items.map((item, index) => {
            ...
            return (
              <div key={item._id} style={{ padding: '5px', height: item.containerHeight, borderLeft: `3px solid ${this.colors[index]}` }}>
                <p><Link to={`/project/${item.project.name.toLowerCase().replace(/ /g, '_')}`}><b>{item.project.name}</b></Link> <span>{startDate.hours}:{startDate.minutes} - {endDate.hours}:{endDate.minutes}</span></p>
